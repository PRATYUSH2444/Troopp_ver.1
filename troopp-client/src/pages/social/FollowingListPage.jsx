...
const FollowingListPage = () => {
  ...
  useEffect(() => {
    const fetchFollowing = async () => {
      try {
        // Mock fetch list: axios.get(`/api/v1/follows/${userId}/following`)
        ...
        console.error('Failed retrieving following list:', err)
      } finally {
        setLoading(false)
      }
    }
    ...
    <div className="w-full max-w-md mx-auto flex flex-col gap-6 pb-20 px-4">
      ...
      {following.length === 0 ? (
        <div className="text-center py-20 bg-stone-50/50 border border-border/80 border-dashed rounded-2xl flex flex-col items-center gap-2">
          ...
          <h4 className="text-xs font-bold text-text-primary">Not following anyone</h4>
          <p className="text-[10px] text-text-secondary max-w-xs leading-relaxed">
            When this explorer follows other co-travelers, they will list up here.
          </p>
        </div>
      ) : (
